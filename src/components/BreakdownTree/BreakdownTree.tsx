...
function TreeContent({
  ...
  const visibleNodes = useMemo(() => {
    ...
    if (state.searchFilter.isActive) {
      return flatNodes.filter(node =>
        ...
        treeUtils.getDescendants(state.tree, node.id).some(descendant =>
  ...
  const handleNodeAction = useCallback(async (action: QuickActionType, nodeId: string) => {
    try {
      switch (action) {
        case 'edit':
        ...
        case 'demote':
          ...
          if (nodeToMove) {
            if (action === 'promote' && nodeToMove.parentId) {
              ...
              const siblings = utils.getSiblings(nodeId);
              const prevSibling = siblings.find(s => s.order < nodeToMove.order);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    switch (action) {
      ...
      case KeyboardAction.MOVE_RIGHT:
        const direction = action.replace('MOVE_', '').toLowerCase() as 'up' | 'down' | 'left' | 'right';
      ...
      case KeyboardAction.SELECT:
        operations.selectNode(focusedNodeId, e.nativeEvent.ctrlKey || e.nativeEvent.metaKey);
        break;
    }
  ...
  const renderNode = useCallback((node: TreeNodeData) => {
    if (enableDragDrop) {
      return (
        <DragDropTreeNode
          key={node.id}
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 border-b bg-card/50 p-4 space-y-4">
        {/* Search and Filters */}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{visibleNodes.length} of {totalNodes} nodes</span>
          </div>
        </div>
      </div>
        ...
        <ScrollArea ref={scrollAreaRef} className="h-full">
          {isLoading ? (
            ...
            >
              ...
              >
                <div className="p-2 space-y-1" role="tree">
